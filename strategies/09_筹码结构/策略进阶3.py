# -*- coding: utf-8 -*-
"""
09 筹码结构 · 进阶3 —— 深度加权解耦（阻力/支撑=筹码重心离现价多深）
====================================================================
【思路来源】
不只数"上方有多少套牢筹码"，而是先用绝对价格算出上方筹码的【加权平均价位(重心)】，
再看这个重心离现价多远——即"套牢盘平均压在现价上方多深"。支撑同理看下方重心的深度。
阻力=头顶重心深度、支撑=脚下重心深度，两者测的是不同的东西，天然解耦
（实测 corr(支撑Z,阻力Z)≈−0.15，远好于进阶1 的 −0.56）。且加权平均自带归一化，
不需要再纠结除以谁。

【进阶3 建仓/平仓逻辑（面板顶部展示）】
1. 逐日维护价格网格上的绝对筹码量（旧筹码按(1−换手率)衰减，新筹码=当日成交额，
   注入到典型价(高+低+收)/3）。
2. 阻力 = 现价上方筹码的加权平均价 / 现价 − 1（上方套牢盘重心的平均深度，%）；
   支撑 = 1 − 现价下方筹码的加权平均价 / 现价（下方获利盘重心的平均深度，%）。
3. 赚钱效应 = 现价 / 全体加权平均持仓成本 − 1（>0 盈利，<0 亏损）。
