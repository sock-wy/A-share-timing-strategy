...
研报未公开参数。建议调整顺序（先对齐信号频率、再对齐净值、最后验收）：
  步骤1 扫 long_ma  —— 对齐【信号次数】≈58
  步骤2 扫 short_ma —— 对齐【次均天数】≈23

启用方法：参照 01_宏观流动性/参数复原.py，取消下方 STEPS 注释并按需微调候选值，
然后 `python strategies/06_期权PCR/参数复原.py` 即可跑分步扫描、生成 复原参数.json。
"""
import sys
...
# —— 待配置：取消注释并按需调整候选值 ——
# STEPS = [("long_ma",[30,45,60,90],"信号次数"),
#          ("short_ma",[5,10,20,30],"次均天数")]

if __name__ == "__main__":
    # run_calibration(策略, STEPS)
    print("【期权PCR】参数复原尚未配置：请先在本文件填写 STEPS 并取消注释。")
