...
数据区间：主数据.xlsx 已拼入 2010-2014 预热段（回测仍锁 2015 起，但长回看窗口的指标
    在 2015 年初即可算出，不再有首年空窗）。例外：离岸人民币汇率自 2012-05 起（Wind CNH
    早段无缓存值），期货基差/期权PCR 无预热（IC 2015-04、期权 2015-02 才上市），
    中债指数/两个 ETF 为备用（当前无策略引用）。
